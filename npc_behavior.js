(() => {
  ...
  function action(n){
    ...
    else if(a==='socialize'){const preferred=n.aiDecision.targetId;const peers=alive().filter(x=>x.id!==n.id&&x.settlementId===n.settlementId);const chosen=peers.find(x=>x.id===preferred)||peers[Math.floor(Math.random()*peers.length)];if(chosen){n.needs.social=clamp(n.needs.social-12);n.mood=clamp(n.mood+2);n.lastAction=`Talked with ${chosen.name}`;window.NPC_MEMORY?.remember(n,`Spent time with ${chosen.name}.`,'relationship',1,chosen.id);}}
    ...
    else if(a==='trade'){n.wealth+=.6;n.needs.wealth=clamp(n.needs.wealth-7);n.lastAction='Completed a trade';}
    ...
    if(completed&&planner?.completeStep) planner.completeStep(n,a);
  }
  function routine(n){
    n.routine=n.routine||{};
    const h=state.hour;
