(() => {
  ...
  function price(s,r){
    if(window.EVERGLEN_MARKETS?.price&&s)return window.EVERGLEN_MARKETS.price(s,r);
  ...
  function householdDemand(n,s){if(!s)return;const r=demandResourceFor(n);s.market=s.market||{demand:{}};s.market.demand=s.market.demand||{};const budget=clamp((n.wealth*.0025+.05)*state.speed,0,.55);if(window.EVERGLEN_MARKETS?.order)window.EVERGLEN_MARKETS.order(s,r,budget,'buy');else s.market.demand[r]=(s.market.demand[r]||0)+budget*.8;const cost=Math.min(n.wealth,budget*price(s,r));if(cost>0){n.wealth-=cost;inventory(s)[r]=Math.max(0,(inventory(s)[r]||0)-budget);}}
  ...
  window.NPC_ECONOMY={price,produce,earn,consume,businessStep,marketStep,tradeStep,demandResourceFor,step:economyStep};
