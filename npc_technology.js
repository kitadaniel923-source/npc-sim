...
(() => {
  ...
  function settlementTech(s){return s?.technology||null;}
  ...
  function compatible(tech,n){
    const skills=tech.skills||[];
    if(skills.includes(n.roleId))return 1.45;
    if(tech.id==='writing'&&n.career?.branch==='education')return 1.3;
    return 1;
  }

  function canResearch(s,id){
    const t=TECHS[id],u=s.technology.unlocked;if(!t||u.includes(id))return false;
    return (t.requires||[]).every(x=>u.includes(x));
  }
  function unlock(s,id){
    const t=TECHS[id];if(!t||s.technology.unlocked.includes(id))return false;
    s.technology.unlocked.push(id);s.technology.discoveries.unshift({id,name:t.name,year:state.year});s.technology.discoveries=s.technology.discoveries.slice(0,24);s.technology.progress[id]=t.cost;
  ...
  function apply(s,people){
    ...
    s.technology.level=clamp(1+Math.floor(u.length/2),1,10);
    s.technology.bonuses={food:0,prosperity:0,construction:0,defense:0,health:0,knowledge:0,administration:0,trade:0,migration:0,medicine:0,reagents:0,education:0,combat:0,warSupply:0,production:0};
    u.forEach(id=>Object.entries(TECHS[id].effects||{}).forEach(([k,v])=>s.technology.bonuses[k]=(s.technology.bonuses[k]||0)+v));
    people.forEach(n=>{ensureNpc(n);n.knowledge=clamp(n.knowledge+(s.technology.bonuses.knowledge||0)*.005);if((s.technology.bonuses.education||0)>0)n.education=clamp(n.education+(s.technology.bonuses.education||0)*.004);
      if(n.roleId==='farmer')n.productionMultiplier=1+(s.technology.bonuses.food||0);
      if(['builder','architect','engineer'].includes(n.roleId))n.constructionMultiplier=1+(s.technology.bonuses.construction||0);
      if(['blacksmith','soldier','captain','general','marshal'].includes(n.roleId))n.combatMultiplier=1+(s.technology.bonuses.combat||0)+(s.technology.bonuses.weapons||0);
      if(['healer','cleric','druid','alchemist'].includes(n.roleId))n.medicineMultiplier=1+(s.technology.bonuses.health||0)+(s.technology.bonuses.medicine||0);
    });
  }
  ...
  function research(s,people){
    ...
    ensureSettlement(s);
    const active=people.filter(n=>n.age>=16&&n.age<=75&&contribution(n,s)>0);
    let points=0;active.forEach(n=>points+=contribution(n,s)*compatible({skills:[]},n));
    points*=1+(s.services?.education||0)*.008;
    if(s.culture?.id==='curiosity')points*=1.18;
    s.technology.points+=points;
    const choices=Object.keys(TECHS).filter(id=>canResearch(s,id));
    choices.sort((a,b)=>{
      const A=TECHS[a],B=TECHS[b];
      const fit=tech=>active.reduce((x,n)=>x+((tech.skills||[]).includes(n.roleId)?2:0)+(tech.id==='writing'&&n.roleId==='scholar'?2:0),0);
      return (fit(B)-fit(A))||(A.localeCompare(B));
    });
    for(const id of choices.slice(0,2)){
      const t=TECHS[id];s.technology.progress[id]=(s.technology.progress[id]||0)+points*.7;
      if(s.technology.progress[id]>=t.cost&&unlock(s,id)){M?.experience?.(active[0],`I contributed to discovering ${t.name}.`,'discovery',3,s.id,'pride',-4,true);break;}
    }
  ...
  function step(){
    ...
    state.settlements.forEach(s=>{const people=alive().filter(n=>n.settlementId===s.id);if(!people.length)return;ensureSettlement(s);if(!touched.has(s.id)&&state.tick%6!==0)return;research(s,people);apply(s,people);if(s.technology.unlocked.includes('navigation'))s.market=s.market||{};if(s.technology.unlocked.includes('printing'))s.infrastructure.knowledge=Math.max(s.infrastructure.knowledge||0,45);if(s.technology.unlocked.includes('masonry'))s.infrastructure.workshops=Math.max(s.infrastructure.workshops||0,25);});
    state.npcs.forEach(ensureNpc);
    state.technology=state.technology||{globalDiscoveries:[]};
  }

  window.NPC_TECHNOLOGY={TECHS,ensureSettlement,unlock,research,step};
