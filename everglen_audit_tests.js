...
(() => {
  ...
  function show(){
    ...
    panel.style.cssText='position:fixed;right:18px;top:18px;z-index:100000;max-width:520px;max-height:80vh;overflow:auto;background:rgba(10,14,20,.97);color:#eef2f7;border:1px solid rgba(255,255,255,.16);border-radius:10px;padding:14px 16px;font:12px/1.45 system-ui,sans-serif;box-shadow:0 12px 40px rgba(0,0,0,.45)';
    ...
    panel.innerHTML=`<div style="font-weight:800;font-size:15px;margin-bottom:5px">Everglen Deep Audit</div><div style="margin-bottom:9px">${report.ok?'ALL INVARIANTS PASS':'INVARIANT FAILURES DETECTED'} · Year ${report.summary.year??'?'} · Tick ${report.summary.tick??'?'}</div>${rows}<div style="margin-top:9px;padding-top:8px;border-top:1px solid rgba(255,255,255,.12)">Population ${report.summary.population} · Settlements ${report.summary.settlements} · Kingdoms ${report.summary.kingdoms} · Families ${report.summary.families} · Systems ${report.summary.systems}</div>`;
