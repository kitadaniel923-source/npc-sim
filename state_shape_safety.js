...
(() => {
  ...
  const ensureArray=(obj,key)=>{if(!Array.isArray(obj[key]))obj[key]=[];return obj[key]};
  function ensure(){
    ...
    const ss=ensureObject(state,'society',society);ensureObject(ss,'culture',culture);ensureObject(ss,'economy',()=>economy(state.food));ensureObject(ss,'identity',identity);ensureArray(ss,'conflicts');
    for(const s of state.settlements||[]){ensureObject(s,'culture',culture);ensureObject(s,'economy',()=>economy(s.wealth));ensureObject(s,'diplomacy',diplomacy);s.history ||= [];s.rumors ||= [];const x=ensureObject(s,'society',society);ensureObject(x,'culture',culture);ensureObject(x,'economy',()=>economy(s.wealth));ensureObject(x,'identity',identity);ensureArray(x,'conflicts');x.institutions ||= {};x.movements ||= [];x.classPower ||= {};x.rivalries ||= []}
    for(const k of state.kingdoms||[]){ensureObject(k,'culture',culture);ensureObject(k,'economy',()=>economy(k.treasury));ensureObject(k,'diplomacy',diplomacy);k.treaties ||= [];k.rumors ||= [];const x=ensureObject(k,'society',society);ensureObject(x,'culture',culture);ensureObject(x,'economy',()=>economy(k.treasury));ensureObject(x,'identity',identity);ensureArray(x,'conflicts')}
