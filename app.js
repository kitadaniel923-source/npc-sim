...
function createNpc(i, factionId, home = null) {
  const spawn = home ? { x: home.x, y: home.y } : { x: Math.random() * 1100 - 550, y: Math.random() * 800 - 400 };
  const npc = {
    id: i + 1,
...
function makeSettlement(foundingNpc, foundingPeople) {
  ...
  const settlement = {
    ...
    homes: Math.max(5, Math.floor(foundingPeople.length / 2)), buildings: 2, food: Math.max(45, state.food), wealth: 120, military: 0, stability: 78,
    culture: 'Founders', history: [`Year ${state.year}: founded by ${foundingNpc.name}.`]
  };
  state.settlements.push(settlement);
  foundingPeople.forEach(n => {
    n.founder = n.id === foundingNpc.id || n.founder;
    n.home = settlement.name;
    n.faction = foundingNpc.faction;
    n.target = { x: settlement.x + (Math.random() - .5) * 70, y: settlement.y + (Math.random() - .5) * 70 };
  });
  foundingNpc.goal = 'Build the village';
  foundingNpc.lastAction = 'Founded a village';
...
function foundVillageIfReady() {
  ...
  // The founder actively calls nearby nomads to the camp. After the first two days,
  // the simulation guarantees a founding party so the civilization chain can actually begin.
  if (state.day >= 2) {
    const candidates = aliveNpcs().filter(n => n.id !== founder.id).sort((a, b) => dist(a, founder) - dist(b, founder)).slice(0, 7);
    candidates.forEach(n => { n.target = { x: founder.x + (Math.random() - .5) * 80, y: founder.y + (Math.random() - .5) * 80 }; n.goal = 'Join the settlers'; });
    if (candidates.length >= settlementRules.village.minPopulation - 1 && state.food >= settlementRules.village.minFood) {
      const foundingParty = [founder, ...candidates];
      makeSettlement(founder, foundingParty);
    }
  }
}
...
function villageProgress(s) {
  ...
  const farmers = people.filter(n => n.job === 'farmer').length;
  s.food = clamp(s.food + farmers * .012 - people.length * .002 * state.speed, 0, 100);
  s.wealth += people.filter(n => ['merchant', 'blacksmith'].includes(n.job)).length * .01 * state.speed;
  s.stability = clamp(s.stability + (s.food > 40 ? .012 : -.04), 0, 100);
...
function tryUpgradeVillage(s) {
  ...
  const p = villageProgress(s), r = settlementRules.city;
  if (p.population >= r.minPopulation && p.adults >= r.minAdults && s.food >= r.minFood && s.homes >= r.minHomes && s.buildings >= r.minBuildings && s.age >= r.minAge) {
    ...
    log(`${s.name} grew from village into a CITY after meeting its requirements.`);
...
function tryCreateSecondSettlement(s) {
  if (s.type !== 'city' || state.settlements.length >= 2) return;
  const people = countSettlementPeople(s);
  if (people.length < 55 || Math.random() > 0.012 * state.speed) return;
  ...
  const newS = {
    ...
    founderId: pioneer.id, rulerId: pioneer.id, age: 0, homes: 4, buildings: 2,
    food: 42, wealth: 55, military: 0, stability: 70, culture: s.culture, history: [`Year ${state.year}: founded by ${pioneer.name}, a citizen of ${s.name}.`]
  };
...
function tryUpgradeToKingdom(s) {
  ...
  const people = countSettlementPeople(s), adults = people.filter(n => n.age >= 18), r = settlementRules.kingdom;
  ...
  const ready = people.length >= r.minPopulation && adults.length >= r.minAdults && s.food >= r.minFood && s.homes >= r.minHomes && s.buildings >= r.minBuildings && s.military >= r.minMilitary && s.stability >= r.minStability && state.settlements.length >= r.minSettlements && totalPeople >= r.minPopulation && leaderEligible;
  if (ready) {
    ...
    const faction = factionOf(leader); faction.name = `Kingdom of ${s.name}`; faction.leader = leader.id; faction.power += 40;
...
function birth(parent) {
  if (aliveNpcs().length >= 230 || parent.age < 20 || parent.age > 45 || Math.random() > .004 * state.speed) return;
  const mates = aliveNpcs().filter(n => n.home === parent.home && n.sex !== parent.sex && n.age >= 20 && n.age <= 42 && n.id !== parent.id);
  const mate = mates[0]; if (!mate) return;
  const faction = factionOf(parent); const home = settlementOf(parent.home);
  const child = createNpc(state.npcs.length, faction.id, home);
  child.age = 0; child.home = parent.home; child.x = parent.x; child.y = parent.y; child.job = 'none'; child.goal = 'Grow up'; child.wealth = 5; child.trait = Math.random() < .5 ? parent.trait : mate.trait;
  ...
  if (Math.random() < .2) log(`${parent.name} and ${mate.name} welcomed ${child.name}.`);
}

function killNpc(npc, reason) {
  if (!npc.alive) return; npc.alive = false; npc.health = 0; log(`${npc.name} died at age ${Math.floor(npc.age)} (${reason}).`); if (state.selected === npc.id) state.selected = null;
}
function maybeDie(npc) {
  if (npc.age > 90 && Math.random() < .0015 * state.speed) killNpc(npc, 'old age');
  else if (npc.health < 1) killNpc(npc, 'illness');
  else if (state.plague && Math.random() < .0005 * state.speed && npc.health < 25) killNpc(npc, 'the plague');
}

function decide(npc) {
  if (!npc.alive) return;
  const home = settlementOf(npc.home);
  if (npc.age < 6) { npc.goal = 'Grow up'; npc.target = home ? { x: home.x, y: home.y } : npc.target; return; }
  if (npc.health < 30) { npc.goal = 'Seek healer'; npc.target = home ? { x: home.x, y: home.y } : npc.target; return; }
  if (npc.hunger > 75) { npc.goal = 'Find food'; npc.target = home ? { x: home.x, y: home.y } : npc.target; return; }
  if (!home) { npc.goal = 'Explore'; if (Math.random() < .4) npc.target = { x: npc.x + Math.random() * 150 - 75, y: npc.y + Math.random() * 150 - 75 }; return; }
  if (home.type === 'kingdom' && npc.trait === 'ambitious' && Math.random() < .001 * state.speed) npc.goal = 'Seek power';
  const hour = state.hour;
  if (hour < 6 || hour >= 22) { npc.goal = 'Sleep'; npc.target = { x: home.x + Math.random() * 35 - 17, y: home.y + Math.random() * 35 - 17 }; }
  else if (hour < 9) { npc.goal = 'Go to work'; npc.target = { x: home.x + Math.random() * 100 - 50, y: home.y + Math.random() * 100 - 50 }; }
  else if (hour < 16) { npc.goal = npc.job === 'farmer' ? 'Farm' : 'Work'; npc.target = { x: home.x + Math.random() * 120 - 60, y: home.y + Math.random() * 120 - 60 }; }
  else if (hour < 19) { npc.goal = Math.random() < .5 ? 'Socialize' : 'Trade'; npc.target = { x: home.x + Math.random() * 90 - 45, y: home.y + Math.random() * 90 - 45 }; }
  else { npc.goal = 'Visit family'; npc.target = { x: home.x, y: home.y }; }
}
...
function updateNpc(npc) {
  ...
  birth(npc); maybeDie(npc);
}

function worldTick() {
  state.tick++; state.hour += .18 * state.speed;
  ...
  if (state.plague) state.weather = 'Plague haze'; else if (state.weather === 'Storm' && Math.random() < .03) state.weather = 'Clear';
  const alive = aliveNpcs();
  state.food = clamp(state.food + alive.filter(n => n.job === 'farmer').length * .006 - alive.length * .0015 * state.speed, 0, 100);
  state.stability = clamp(state.stability + (state.food > 35 ? .012 : -.07) * state.speed, 0, 100);
  foundVillageIfReady(); assignSettlementPeople();
...
function updateFactions() { state.factions.forEach(f => { f.population = aliveNpcs().filter(n => n.faction === f.id).length; f.power = clamp(20 + f.population * .45, 0, 100); }); }
...
function triggerEvent(name) {
  ...
  if (name === 'meteor') { state.food = clamp(state.food - 5, 0, 100); state.stability = clamp(state.stability - 6, 0, 100); log('A meteor strikes the wilderness. The population watches in horror.'); }
  renderAll();
}

function renderAll() { renderPanels(); drawWorld(); renderInspector(); }
function renderPanels() {
  const alive = aliveNpcs();
  elements.year.textContent = state.year; elements.season.textContent = state.season; elements.population.textContent = alive.length;
  elements.families.textContent = alive.filter(n => n.children > 0).length; elements.settlements.textContent = state.settlements.length;
  elements.food.textContent = `${Math.round(state.food)}%`; elements.stability.textContent = `${Math.round(state.stability)}%`; elements.tickLabel.textContent = `tick ${state.tick}`;
  elements.speedLabel.textContent = `${state.speed}×`; elements.worldStatus.textContent = state.war ? 'War' : state.plague ? 'Crisis' : state.settlements[0]?.type === 'kingdom' ? 'Kingdom era' : 'Peaceful';
  elements.worldStats.innerHTML = state.settlements.length ? state.settlements.map(s => { const people = countSettlementPeople(s); return `<div class="metric"><span>${s.type.toUpperCase()} · ${s.name}</span><b>${people.length} pop · ${s.homes} homes</b></div>`; }).join('') : '<div class="empty-state">The world has no settlements yet. Ari is gathering the first settlers.</div>';
...
function renderInspector() {
  ...
  if (!npc) { elements.inspectorContent.innerHTML = '<div class="empty-state">Select an NPC to inspect their life, memories, relationships, and current goal.</div>'; elements.selectedNeedMood.textContent = 'No selection'; elements.needsChart.innerHTML = ''; return; }
  const f = factionOf(npc), s = settlementOf(npc.home);
  elements.inspectorContent.innerHTML = `<div class="person-head"><div class="portrait" style="background:${npc.color}">${npc.name[0]}</div><div><h3>${npc.name}</h3><span>${Math.floor(npc.age)} years · ${npc.job} · ${f ? f.name : 'Independent'}</span></div></div><div class="goal"><strong>Current goal:</strong> ${npc.goal}</div><div class="bio-grid"><span>Home<b>${s ? s.name : 'Nomad'}</b></span><span>Trait<b>${npc.trait}</b></span><span>Wealth<b>${Math.floor(npc.wealth)}</b></span><span>Children<b>${npc.children}</b></span></div><h4>Memories</h4><div class="memories">${(npc.memories.length ? npc.memories : [{ text: 'No major memories yet.', year: state.year }]).map(m => `<div>Y${m.year} · ${m.text}</div>`).join('')}</div>`;
...
function resize() { const r = canvas.getBoundingClientRect(), d = window.devicePixelRatio || 1; canvas.width = Math.floor(r.width * d); canvas.height = Math.floor(r.height * d); ctx.setTransform(d, 0, 0, d, 0, 0); }
...
function drawWorld() {
  const w = canvas.clientWidth, h = canvas.clientHeight; ctx.clearRect(0, 0, w, h); ctx.fillStyle = '#10251d'; ctx.fillRect(0, 0, w, h);
  ...
  state.settlements.forEach(s => {
    ...
    ctx.beginPath(); ctx.arc(p.x, p.y, r, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = s.color + '88'; ctx.lineWidth = s.type === 'kingdom' ? 3 : 2; ctx.stroke();
  ...
  aliveNpcs().forEach(n => { const p = worldToScreen(n.x, n.y), size = n.id === state.selected ? 9 : 6; ctx.fillStyle = n.color; ctx.beginPath(); ctx.arc(p.x, p.y, size, 0, Math.PI * 2); ctx.fill(); if (n.id === state.selected) { ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(p.x, p.y, 13, 0, Math.PI * 2); ctx.stroke(); } if (state.showNames && state.camera.zoom > .75) { ctx.fillStyle = 'rgba(255,255,255,.78)'; ctx.font = '10px Segoe UI'; ctx.fillText(n.name, p.x + 8, p.y + 3); } });
}
...
function reset() {
  ...
  placeInitialNomads(); log('A new world begins. There are no cities, no kingdoms, and no borders.'); log('Ari, the first NPC, will gather settlers and found the first village.'); renderAll();
}
...
function setupControls() {
  ...
  canvas.addEventListener('click', e => { if (state.dragging) return; const rect = canvas.getBoundingClientRect(), m = screenToWorld(e.clientX - rect.left, e.clientY - rect.top); let closest = null, best = 16 / state.camera.zoom; aliveNpcs().forEach(n => { const d = Math.hypot(n.x - m.x, n.y - m.y); if (d < best) { best = d; closest = n; } }); state.selected = closest ? closest.id : null; renderAll(); });
