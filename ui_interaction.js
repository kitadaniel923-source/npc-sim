...
(() => {
  ...
  function addPanelControls(){document.querySelectorAll('.side .panel').forEach(panel=>{if(panel.dataset.everglenUiReady==='1')return;panel.dataset.everglenUiReady='1';const title=panel.querySelector('.panel-title');if(!title)return;const button=document.createElement('button');button.type='button';button.className='everglen-panel-toggle';button.textContent='×';button.title='Hide panel';button.setAttribute('aria-label','Hide panel');button.style.cssText='margin-left:auto;min-width:22px;padding:2px 5px;font-size:10px;line-height:1;cursor:pointer;';button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();togglePanel(panel,button);});title.appendChild(button);});}
