// Resource registry: taxonomy, inventories and recipes. Economy owns production execution.
(() => {
  ...
  const OUTPUT = {
    ...
    miner:{iron:2,silver:.08,gold:.02,mithril:.004,adamantine:.001,emberite:.002,moonstone:.0015,voidstone:.0005,starsteel:.0001}, woodcutter:{wood:3.0}, mason:{stone:2.2}, builder:{tools:.08},
    blacksmith:{weapons:.5,armor:.22,tools:.65,iron:-2,starsteel:-.002,adamantine:-.004,mithril:-.006,emberite:-.003}, carpenter:{tools:.45,boats:.04,wood:-2}, weaver:{cloth:1.2,wool:-.65},
    baker:{food:2.2,grain:-1.1}, cook:{food:1.6,grain:-.35,livestock:-.015}, brewer:{grain:-.5,food:.25}, herbalist:{herbs:1.1,medicine:.12}, healer:{medicine:.7,herbs:-.2}, doctor:{medicine:1.0,herbs:-.35},
    merchant:{gold:.5}, trader:{gold:.85}, peddler:{gold:.3}, shopkeeper:{gold:.35}, farrier:{horses:.03,tools:.15,iron:-.3}, sailor:{fish:.45,boats:.01,gold:.2}, shipwright:{boats:.08,battle_boats:.025,wood:-3.2,tools:-.3,iron:-.4},
    scholar:{paper:.2,books:.08}, teacher:{paper:-.08}, scribe:{paper:-.2,books:.08}, librarian:{books:.05}, artist:{cloth:.05,gold:.15}, musician:{gold:.18}, courier:{horses:.02,paper:.03}, engineer:{tools:-.25,stone:.3,wood:.25}, architect:{paper:-.08,stone:.12},
    explorer:{paper:.05,gold:.08}, beastmaster:{livestock:.05,horses:.03}, furniture_maker:{wood:-1.8,tools:-.2,gold:.25}, alchemist:{medicine:.45,reagents:-1.0,herbs:-.25,emberite:-.01,moonstone:-.006},
    enchanter:{armor:.08,reagents:-1.1,mithril:-.004,moonstone:-.01,voidstone:-.003}, mage:{reagents:-.35,moonstone:-.006,voidstone:-.002}, wizard:{reagents:-.55,moonstone:-.01,voidstone:-.004,starsteel:-.001}, druid:{herbs:.65,medicine:.25,food:.35}, cleric:{medicine:.3,food:.1},
    cavalry:{horses:-.02,food:-.3,armor:-.05,weapons:-.04}, soldier:{food:-.22,weapons:-.08,armor:-.05}, knight:{food:-.3,weapons:-.1,armor:-.08,mithril:-.001}, archer:{food:-.22,weapons:-.06}, spearman:{food:-.22,weapons:-.07}, militia:{food:-.15,weapons:-.04}, captain:{food:-.25,weapons:-.08,armor:-.05}, general:{food:-.25,weapons:-.06,armor:-.04}, marshal:{food:-.3,weapons:-.08,armor:-.05,battle_boats:-.001}
  };

  function ensureSettlement(s){s.resources=s.resources||{};Object.keys(RESOURCES).forEach(id=>{if(s.resources[id]==null)s.resources[id]=0;});return s;}
  function ensureNpc(n){n.inventory=n.inventory||{};Object.keys(RESOURCES).forEach(id=>{if(n.inventory[id]==null)n.inventory[id]=0;});return n;}
  function catalog(){return RESOURCES;}
  function price(s,id){const r=RESOURCES[id];if(!r)return 10;const stock=s?.resources?.[id]??0;const population=Math.max(1,(state.npcs||[]).filter(n=>n.alive&&n.settlementId===s?.id).length);const target=Math.max(8,population*.35);const scarcity=Math.max(-.45,Math.min(1.4,(target-stock)/target));return Math.max(1,r.base*(1+scarcity));}

  // Compatibility helper. This no longer runs automatically, preventing duplicate production.
  function applyProduction(n,s,outputScale=.03){ensureNpc(n);ensureSettlement(s);const recipe=OUTPUT[n.roleId];if(!recipe)return;Object.entries(recipe).forEach(([id,value])=>{if(!RESOURCES[id]||typeof value!=='number')return;const amount=value*state.speed*outputScale;if(amount>=0)s.resources[id]=(s.resources[id]||0)+amount;else{s.resources[id]=Math.max(0,(s.resources[id]||0)-Math.min(s.resources[id]||0,Math.abs(amount)));}});}

  function step(){if(!state.running)return;state.settlements?.forEach(ensureSettlement);state.npcs?.filter(n=>n.alive).forEach(ensureNpc);}
