...
(() => {
  ...
  function familyEconomySection(n,family){
    const api=window.EVERGLEN_FAMILY_ECONOMY;if(!api||!family)return '';
    const estate=family.estate||{},assets=family.assets||{};
    const heir=family.primaryHeirId?state.npcs.find(x=>x.id===family.primaryHeirId):null;
    const household=n.householdId||'Unassigned';
    const role=n.householdRole||'member';
    const net=Number(family.netWorth||0),power=Number(family.economicPower||0),cashflow=Number(family.familyCashflow||0);
    return `<h4>Family economy</h4><div class="bio-grid"><span>Household<b>${esc(household)}</b></span><span>Household role<b>${esc(role)}</b></span><span>Family net worth<b>${Math.floor(net)}g</b></span><span>Economic power<b>${Math.round(power)}%</b></span><span>Family cashflow<b>${cashflow>=0?'+':''}${cashflow.toFixed(1)}g</b></span><span>Property value<b>${Math.floor(assets.propertyValue||family.propertyValue||0)}g</b></span><span>Land<b>${assets.land||family.land||0}</b></span><span>Buildings<b>${assets.buildings||0}</b></span><span>Businesses<b>${family.businesses||0}</b></span><span>Dependents<b>${family.dependents||0}</b></span><span>Poor members<b>${family.poorMembers||0}</b></span><span>Primary heir<b>${esc(heir?.name||'None')}</b></span></div>`;
  }
