// AI inspector overlay: exposes the selected NPC's life, social rank and decision state.
(() => {
  ...
  function socialSection(n,settlement){
    const society=window.MEDIEVAL_SOCIETY,rank=n.socialClass&&society?.RANKS?.[n.socialClass];const guild=n.guildId&&society?.GUILD_ROLES?.[n.guildId];
  ...
  function renderInspector(){
    ...
    const kingdom=state.kingdoms?.find(k=>k.id===n.kingdomId)||state.kingdoms?.find(k=>k.id===n.faction);const settlement=state.settlements.find(s=>s.id===n.settlementId);const family=state.families.find(f=>f.id===n.familyId);const memories=Array.isArray(n.memories)?n.memories:[],needs=n.needs||{},career=n.careerHistory?.[n.careerHistory.length-1]||n.career||n.roleName,culture=n.culture||n.cultureId||(kingdom&&(kingdom.culture||kingdom.cultureId));
    const political=kingdom?.politics||{};const politicalBlock=kingdom?`<div class="goal"><b>Political pressure:</b> Nobles ${Math.round(political.nobles||0)} · Merchants ${Math.round(political.merchants||0)} · Commons ${Math.round(political.commons||0)} · Army ${Math.round(political.army||0)}</div>`:'';
    const crimeBlock=n.crimeHeat>0||n.crimes>0||n.arrests>0?`<div class="goal"><b>Crime:</b> heat ${Math.round(n.crimeHeat||0)} · crimes ${n.crimes||0} · arrests ${n.arrests||0}${n.term?` · sentence ${esc(n.term)}y`:''}</div>`:'';
