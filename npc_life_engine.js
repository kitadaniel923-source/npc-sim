(() => {
  ...
  const INHERIT = {
    ...
    dwarf:['strong','hardy'], giant:['strong','tough'], hardy:['tough','disease_resistant'],
    blessed:['lucky'], cursed:['unlucky'], genius:['smart','logical','scholarly'], veteran:['disciplined','battle_hardened']
  };
  const EXPERIENCE = [
  ...
  function inheritTraits(child,mother,father){
    ...
    if(!child.traits.length) child.trait=child.trait||'curious';
    child.trait=child.traits[0]||child.trait;
    const f=family(child);
    if(f) f.bloodlineTraits=[...new Set([...(f.bloodlineTraits||[]),...child.traits.filter(t=>GENETIC.has(t))])].slice(0,12);
    child._traitsInherited=true;
  }

  function locateParents(n){ return (n.parentIds||[]).map(id=>state.npcs.find(x=>x.id===id)).filter(Boolean); }
  function birthSync(){ for(const child of alive()) if(child.parentIds?.length && !child._traitsInherited){ const p=locateParents(child); inheritTraits(child,p[0],p[1]); } }
  ...
  function eventTraitDevelopment(n){
    ...
    for(const e of EXPERIENCE){
      ...
      if(e.traits.length){n.lastAction=e.text;log(`${n.name} ${e.text}`);}
    }
    if(has(n,'brave')&&has(n,'cowardly')&&Math.random()<.015)n.innerConflict='Courage versus fear';
  }

  function needsDevelopment(n){
    const dangerous=state.war||state.weather==='Storm'||n.health<45;
    if(dangerous&&!has(n,'resourceful')&&(n.wealth<15||n.hunger>65)&&Math.random()<.02)add(n,'resourceful');
  ...
  function relationshipStep(n){
    const candidates=other(n,x=>x.settlementId===n.settlementId).slice(0,18); if(!candidates.length)return;
    const target=candidates[Math.floor(Math.random()*candidates.length)]; n.relationships=n.relationships||[]; target.relationships=target.relationships||[];
    let a=n.relationships.find(x=>x.id===target.id),b=target.relationships.find(x=>x.id===n.id);
    if(!a){a={id:target.id,score:50};n.relationships.push(a)} if(!b){b={id:n.id,score:50};target.relationships.push(b)}
    let delta=0;
  ...
  function socialCompatibility(n,target){
    let score=50;
    const pairs=[['friendly','friendly',12],['kind','empathetic',12],['honest','honest',10],['loyal','loyal',12],['charismatic','shy',5],['romantic','romantic',15],['protective','loyal',8],['calm','calm',12],['deceptive','honest',-18],['cruel','kind',-20],['paranoid','trusting',-18],['antisocial','extrovert',-12],['jealous','romantic',-2],['ambitious','ambitious',-4]];
    for(const [a,b,v] of pairs)if((has(n,a)&&has(target,b))||(has(n,b)&&has(target,a)))score+=v;
    return score;
  }
  function marriageCompatibility(n,target){ return socialCompatibility(n,target)+(n.wealth>100?7:0)+(target.wealth>100?7:0)+((n.status||0)>60?8:0)+((target.status||0)>60?8:0); }

  function nobilityStep(n){
    const role=n.roleId, nobleRoles=['baron','count','duke','king','heir','emperor'];
    const noble=nobleRoles.includes(role)||n.classTier==='noble'||n.classTier==='royal'||has(n,'noble_born')||has(n,'royal_blood');
    if(noble)add(n,'noble_born'); if(role==='king'||role==='emperor')add(n,'royal_blood'); if(role==='heir')add(n,'legitimate_heir');
    if((n.status||0)>78)add(n,'prestigious'); if((n.status||0)>88)add(n,'influential');
    if(n.roleId==='duke'||n.roleId==='count'){if(has(n,'ambitious'))add(n,'ambitious_noble');if(has(n,'deceptive')||has(n,'manipulative'))add(n,'scheming_noble');if(has(n,'honest')||has(n,'loyal'))add(n,'honorable_noble');}
    const k=kingdom(n); if(!k)return;
    n.successionClaim=clamp(n.successionClaim+(has(n,'royal_blood')?1.5:0)+(has(n,'strong_claim')?1:0)-(has(n,'weak_claim')?.2:0));
  ...
  function inheritOnDeath(dead){
    ...
    const heirs=alive().filter(x=>x.familyId===dead.familyId&&x.id!==dead.id).sort((a,b)=>{
      const ca=(dead.childrenIds||[]).includes(a.id)?4:0,cb=(dead.childrenIds||[]).includes(b.id)?4:0;
      const ga=(a.parentIds||[]).includes(dead.id)?3:0,gb=(b.parentIds||[]).includes(dead.id)?3:0;
      return (cb+gb)-(ca+ga)||(b.status||0)-(a.status||0)||b.age-a.age;
    });
    const heir=heirs[0];
    if(heir){heir.wealth+=(dead.wealth||0)*.75;heir.status=clamp((heir.status||0)+Math.max(2,(dead.status||0)*.05));heir.successionClaim=clamp((heir.successionClaim||0)+8);add(heir,'favored_heir');}
  }

  function economyStep(n){
    if(n.age<16)return;
    const rich=n.wealth>=120;if(rich)add(n,'wealthy');else remove(n,'wealthy');
    if(n.wealth<12)add(n,'poor');else remove(n,'poor');
    if(has(n,'lazy')&&rich)n.workEthic=Math.max(5,n.workEthic-1);
    if(has(n,'entrepreneur')&&n.wealth>25&&Math.random()<.02){n.businessCount=(n.businessCount||0)+1;n.wealth+=rand(2,8);n.reputation=clamp((n.reputation||50)+.4);}
    ...
    if(has(n,'frugal'))n.wealth+=.25;if(has(n,'wasteful'))n.wealth=Math.max(0,n.wealth-.35);
  }

  function combatStep(n){
    if(n.age<14)return;
    if(has(n,'fearless'))n.combatSkill+=.08;if(has(n,'veteran'))n.combatSkill+=.07;if(has(n,'battle_hardened'))n.combatSkill+=.05;if(has(n,'tactical_fighter'))n.combatSkill+=.06;if(has(n,'inexperienced')&&Math.random()<.01)n.combatSkill=Math.max(0,n.combatSkill-.1);
    if(state.war&&has(n,'cowardly')){n.goal='Avoid the front';n.mood=clamp((n.mood||50)-.2);} if(state.war&&(has(n,'aggressive')||has(n,'brave'))){n.goal='Seek battle';n.combatSkill+=.06;}
  }

  function intelligenceStep(n){
    if(has(n,'genius'))n.education=clamp((n.education||0)+.05); if(has(n,'smart')||has(n,'logical'))n.education=clamp((n.education||0)+.035); if(has(n,'slow')||has(n,'forgetful'))n.education=clamp((n.education||0)-.012); if(has(n,'focused'))n.education=clamp((n.education||0)+.02); if(has(n,'distractible'))n.education=clamp((n.education||0)-.018); if(has(n,'observant'))n.suspicion=clamp((n.suspicion||0)+.02);
  }

  function civilizationStep(){
    for(const k of state.kingdoms||[]){
      const people=alive().filter(n=>n.faction===k.id);if(!people.length)continue;
      let productivity=0,peace=0,war=0,learning=0,crime=0;
      for(const n of people){productivity+=(has(n,'hardworking')?2:0)+(has(n,'disciplined')?1.5:0)+(has(n,'builder')?1:0);peace+=(has(n,'calm')?1.5:0)+(has(n,'peacemaker')?2:0)+(has(n,'cooperative')?1:0);war+=(has(n,'veteran')?1.5:0)+(has(n,'strategic')?1:0)+(has(n,'aggressive')?1:0);learning+=(has(n,'genius')?2:0)+(has(n,'scholarly')?1.5:0)+(has(n,'creative')?1:0);crime+=(has(n,'criminal')?2:0)+(has(n,'deceptive')?1:0)+(has(n,'cruel')?1:0);}
      const size=Math.max(1,people.length);k.stability=clamp((k.stability||70)+((peace-crime)/size)*.08+(state.war?-0.02:0));k.power=Math.max(0,(k.power||20)+(productivity/size)*.025+(war/size)*.02+(learning/size)*.018);k.treasury=Math.max(0,(k.treasury||0)+(productivity/size)*.12-(crime/size)*.04);
    }
  }

  function dynastyHistory(){
    if(!state._lifeHistory)state._lifeHistory=[];const events=state._lifeHistory;
    for(const n of alive()){
      const combo=n.lastTraitCombo;if(combo&&(!n._loggedCombo||n._loggedCombo!==combo.id)){events.unshift({year:state.year,npc:n.name,type:'trait',text:`${n.name} became known as ${combo.title}.`});n._loggedCombo=combo.id;}
      if((n.roleId==='king'||n.roleId==='queen'||n.roleId==='emperor'||n.roleId==='empress')&&n._lastLoggedRole!=='ruler'){events.unshift({year:state.year,npc:n.name,type:'ruler',text:`${n.name} became ruler of ${kingdom(n)?.name||'the realm'}.`});n._lastLoggedRole='ruler';}
    }
    state._lifeHistory=events.slice(0,120);window.EVERGLEN_HISTORY=state._lifeHistory;
  }

  function step(){
    if(!state.running)return;
    birthSync();
    for(const n of alive()){ensureBase(n);eventTraitDevelopment(n);needsDevelopment(n);/* relationships, economy and combat are owned by dedicated systems now */intelligenceStep(n);nobilityStep(n);}
    civilizationStep();dynastyHistory();
  }

  window.EVERGLEN_LIFE_ENGINE={step,inheritTraits,socialCompatibility,marriageCompatibility,inheritOnDeath};
  if(state.registerSystem) state.registerSystem({name:'life_engine',step,priority:35});
  step();
})();
