(() => {
  ...
  function culturalEffects(n,s){
    ensureNpc(n);
    const c=n.culture,id=c?.id;
    const strength=clamp(c?.strength||45,0,100)/100;
    if(id==='tradition'){
      n.loyalty=clamp((n.loyalty||50)+.015*strength);n.grievance=clamp((n.grievance||0)-.01*strength);
      if(n.familyId)n.needs&&(n.needs.belonging=clamp((n.needs.belonging||50)-.04*strength));
    }
    if(id==='honor'){
      n.honor=clamp((n.honor||50)+.02*strength);n.combatSkill=clamp((n.combatSkill||50)+.015*strength);
      if(state.war&&has(n,'brave'))n.mood=clamp((n.mood||50)+.02*strength);
    }
    if(id==='curiosity'){
      n.education=clamp((n.education||0)+.018*strength);n.goal=(score(n,'curiosity')>70?'Explore':n.goal);
      n.needs&&(n.needs.purpose=clamp((n.needs.purpose||50)-.02*strength));
    }
    if(id==='prosperity'){
      n.fortune=clamp((n.fortune||50)+.02*strength);
      if(['merchant','trader'].includes(n.roleId))n.influence=clamp((n.influence||0)+.025*strength,0,50);
    }
    if(id==='compassion'){
      n.mood=clamp((n.mood||50)+.018*strength);
      if(n.grievance>20)n.grievance=clamp(n.grievance-.02*strength);
    }
    if(id==='strength'){
      n.courageScore=clamp((n.courageScore||50)+.02*strength);
      n.needs&&(n.needs.safety=clamp((n.needs.safety||50)+.015*strength));
    }
    if(s&&c.belief===s.culture?.belief)n.beliefStrength=clamp((n.beliefStrength||40)+.015*strength);
  }
  ...
  function step(){
    ...
    state.families.forEach(f=>{
      const members=people.filter(n=>n.familyId===f.id);if(!members.length)return;
      const lead=members.slice().sort((a,b)=>(b.age||0)-(a.age||0))[0];
      f.culture=lead.culture?.id||f.culture||'tradition';f.belief=lead.culture?.belief||f.belief||'Ancestor Keepers';f.traditions=(lead.culture?.history||[]).slice(-4);
    });
