...
(() => {
  ...
  function draw() {
    ...
    if (state.camera?.feedbackNudge && active.x != null && active.y != null) {
      if (now <= state.camera.feedbackNudge.until) {
        ...
        state.camera.x = active.cameraStartX;
        state.camera.y = active.cameraStartY;
    ...
    if (!toast) {
      toast=document.createElement('div'); toast.id='eventFeedbackToast';
    ...
    if(canvas&&canvas.width){
      const ctx=canvas.getContext('2d'),pulse=Math.sin(age/120*Math.PI)*.5+.5,alpha=(1-age/2300)*(.16+pulse*.12);
      ctx.save();
  ...
  function setEnabled(value) {
    ...
    if(!enabled){active=null;if(state.camera?.feedbackNudge){state.camera.x=state.camera.feedbackNudge.originX??state.camera.x;state.camera.y=state.camera.feedbackNudge.originY??state.camera.y;state.camera.feedbackNudge=null;}const toast=document.getElementById('eventFeedbackToast');if(toast)toast.remove();}
    window.SIM_RENDER?.();
  }
  ...
  function injectToggle(){
    ...
    const button=document.createElement('button');button.id='toggleFeedback';button.textContent=enabled?'✦ Feedback':'✦ Feedback Off';button.title='Toggle major-event screen feedback';
    button.addEventListener('click',()=>{setEnabled(!enabled);button.textContent=enabled?'✦ Feedback':'✦ Feedback Off';});controls.appendChild(button);
  }
