(() => {
  ...
  function interact(a,b,context='neutral',intensity=1){
    ...
    if(delta>=0){r.positive=(r.positive||0)+1;q.positive=(q.positive||0)+1;}
    else{r.negative=(r.negative||0)+1;q.negative=(q.negative||0)+1;}
  ...
  function classify(a,b,r){
    ...
    if((a.parentIds||[]).includes(b.id)||(a.childrenIds||[]).includes(b.id)){
      r.type=a.parentIds?.includes(b.id)?'child':'parent';
      return;
    }
  ...
  function chooseTarget(n){
    ...
    return peers.sort((a,b)=>{
      const rb=relation(n,b.id),ra=relation(n,a.id);
      const kinA=familyRelation(n,a),kinB=familyRelation(n,b);
  ...
  function socialEvent(n){
    ...
    if(p.aggression>72&&(rel?.score||0)<20&&kin<60&&Math.random()<.12){
  ...
  function decay(){
    alive().forEach(n=>(n.relations||[]).forEach(r=>{const age=state.tick-(r.lastTick||state.tick);const kin=(n.familyId&&state.npcs.find(x=>x.id===r.targetId)?.familyId===n.familyId)?1:0;if(age>40)r.score=clamp(r.score-.015*Math.min(age-40,200)+kin*.006);if(age>90)r.familiarity=clamp(r.familiarity-.03,0,100);if(age>150&&!kin)r.trust=clamp((r.trust||50)-.01,0,100);}));
  }

  function step(){
    if(!state.running)return;
    alive().forEach(n=>{n.relationshipProfile=n.relationshipProfile||{};n.relationshipProfile.sociability=score(n,'sociability');n.relationshipProfile.trust=score(n,'loyalty');n.relationshipProfile.aggression=score(n,'aggression');n.relationshipProfile.familyAttachment=n.familyId?68:0;});
